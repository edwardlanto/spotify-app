...
function SongRow({ track }) {
  ...
  const playButton = (trackParams) => {
    if(spotify.audio.is_playing === true && trackParams.preview_url === spotify.audio.src){
      return (
        <IconButton onClick={() => pause()}>
        <PauseIcon />
      </IconButton>
      )
    }else if(spotify.audio.is_playing === false && trackParams.preview_url !== spotify.audio.src){
      return (
